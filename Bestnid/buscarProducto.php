						...
						<li>
							<div class="dropdown">
								 ...
								 <ul class="dropdown-menu" aria-labelledby="dropdownMenu1">
								  	<li><a href='<?php echo "buscarProducto.php?input_buscar=".$_GET["input_buscar"]."&order=fecha_cierre ASC"; ?>'>Ascendente</a></li>
								  	<li><a href='<?php echo "buscarProducto.php?input_buscar=".$_GET["input_buscar"]."&order=fecha_cierre DESC"; ?>'>Descendente</a></li>   
								 </ul>
							</div>
						</li>
						<li>
							<div class="dropdown">
								 ...
								 <ul class="dropdown-menu" aria-labelledby="dropdownMenu1">
								  	<li><a href='<?php echo "buscarProducto.php?input_buscar=".$_GET["input_buscar"]."&order=nombre ASC"; ?>'>Ascendente</a></li>
								  	<li><a href='<?php echo "buscarProducto.php?input_buscar=".$_GET["input_buscar"]."&order=nombre DESC"; ?>'>Descendente</a></li>   
								 </ul>
							</div>
						</li>
						<li>
							<div class="dropdown">
								 ...
								 <ul class="dropdown-menu" aria-labelledby="dropdownMenu1">
								  	<li><a href='<?php echo "buscarProducto.php?input_buscar=".$_GET["input_buscar"]."&order=fecha_realizacion ASC"; ?>'>Ascendente</a></li>
								  	<li><a href='<?php echo "buscarProducto.php?input_buscar=".$_GET["input_buscar"]."&order=fecha_realizacion DESC"; ?>'>Descendente</a></li>   
								 </ul>
							</div>
						</li>
					</ul>	
		        	<?php			          
						...
						if (!isset($_GET["order"]) || ($_GET["order"]!="estado ASC" && $_GET["order"]!="estado DESC" && $_GET["order"]!="fecha_cierre ASC" && $_GET["order"]!="fecha_cierre DESC" && $_GET["order"]!="nombre ASC" && $_GET["order"]!="nombre DESC" && $_GET["order"]!="fecha_realizacion ASC" && $_GET["order"]!="fecha_realizacion DESC") ) {
							$result = mysqli_query ($link, "SELECT Subasta.idSubasta,Subasta.fecha_cierre,Subasta.estado,Subasta.fecha_realizacion,Subasta.idUsuario, Producto.imagen, Producto.nombre, Producto.descripcion, Categoria.nombre AS nomCat 
								...
								WHERE (Producto.nombre like '%".$_GET["input_buscar"]."%' or Producto.descripcion like '%".$_GET["input_buscar"]."%') and Subasta.estado='activa'");
						}
						else {
							$result = mysqli_query ($link, "SELECT Subasta.idSubasta,Subasta.fecha_cierre,Subasta.estado,Subasta.fecha_realizacion,Subasta.idUsuario, Producto.imagen, Producto.nombre, Producto.descripcion, Categoria.nombre AS nomCat 
								FROM Subasta INNER JOIN Producto ON Subasta.idProducto=Producto.idProducto 
								INNER JOIN Categoria ON Producto.idCategoria=Categoria.idCategoria 
								WHERE (Producto.nombre like '%".$_GET["input_buscar"]."%' or Producto.descripcion like '%".$_GET["input_buscar"]."%') and Subasta.estado='activa' ORDER BY ".$_GET["order"]." ");
						}
						...
						while ($row=mysqli_fetch_array($result) ) {
							...
							echo "<div class='panel panel-default row'>
    								...
    								if (isset($_SESSION["idUsuario"]) && $row["estado"]=="activa" && $row["idUsuario"]==$_SESSION["idUsuario"]) {
    									echo "
    										 <div class='row'>
    								...
    								if (isset($_SESSION["idUsuario"]) && $row["estado"]!="cerrada" && ($row["idUsuario"]==$_SESSION["idUsuario"] || $_SESSION["admin"]) ) {
    									echo "
    										 <div class='row'>
